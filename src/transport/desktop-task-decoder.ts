...
export function readDesktopChange(
  ...
  if (change?.type === "snapshot") {
    ...
    return {
      ...
      change: {
        ...
        deliveryBindings: snapshotDeliveryBindings(entities),
      },
    };
  }
  ...
  return {
    ...
    change: {
      ...
      deliveryBindings: patches.flatMap(patchDeliveryBindings),
    },
  };
}

function snapshotDeliveryBindings(
  entities: Record<string, unknown>,
): ReadonlyArray<{ readonly key: string; readonly deliveryId: string }> {
  return Object.entries(entities).flatMap(([key, entity]) =>
    deliveryIds(entity).map((deliveryId) => ({ key, deliveryId }))
  );
}
...
function patchDeliveryBindings(
  value: unknown,
): ReadonlyArray<{ readonly key: string; readonly deliveryId: string }> {
  const patch = record(value);
  const path = Array.isArray(patch?.path) ? patch.path : [];
  ...
  const key = marker < 0 ? undefined : path[marker + 1];
  if (typeof key !== "string") return [];
  return patchDeliveryIds(value).map((deliveryId) => ({ key, deliveryId }));
}
